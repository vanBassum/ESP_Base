...
public:
    ...
    public:
        ...
        Proxy operator[](const std::string_view& subkey) { return provider->GetProxy(subkey); }
    ...
    Proxy operator[](const std::string_view& key) { return GetProxy(key); }
    ...
    virtual Result Get(std::string_view* value)                  { ESP_LOGE("IConfigProvider", "Not implemented get for type 'std::string_view'"); return Result::NotSupported; }
    ...
    virtual Result Set(const std::string_view value)             { ESP_LOGE("IConfigProvider", "Not implemented set for type 'std::string_view'"); return Result::NotSupported; }

protected:
    virtual Proxy GetProxy(const std::string_view& key) = 0;
};
